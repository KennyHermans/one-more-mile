-- Update the upgrade_sensei_level function to automatically set can_create_trips for Master Sensei
...
BEGIN
  ...
  -- Update sensei level and permissions
  ...
  SET 
    ...
    -- Set can_create_trips to true for Master Sensei
    can_create_trips = CASE 
      WHEN p_new_level = 'master_sensei' THEN true 
      ELSE can_create_trips 
    END
  WHERE id = p_sensei_id;
...
-- Update Kenny Hermans to have trip creation permissions as Master Sensei
UPDATE sensei_profiles 
SET can_create_trips = true 
WHERE name = 'Kenny Hermans' AND sensei_level = 'master_sensei';