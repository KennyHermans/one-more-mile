...
-- Fix function search path for calculate_enhanced_sensei_match_score
DROP FUNCTION IF EXISTS public.calculate_enhanced_sensei_match_score(UUID, TEXT, TEXT, TEXT[], UUID);

CREATE FUNCTION public.calculate_enhanced_sensei_match_score(
  p_sensei_id UUID, 
  p_trip_theme TEXT, 
...
BEGIN
  ...
  IF p_destination IS NOT NULL THEN
    ...
    FROM public.destination_skill_mappings dsm
  ...
  SELECT 
    ...
    (
      ...
      CASE WHEN dest_mapping.id IS NOT NULL THEN
        (SELECT COALESCE(SUM(2.0), 0) FROM public.sensei_skills ss 
         WHERE ss.sensei_id = sp.id AND ss.is_active = TRUE 
...
-- Fix function search path for calculate_enhanced_sensei_insights
DROP FUNCTION IF EXISTS public.calculate_enhanced_sensei_insights(UUID);

CREATE FUNCTION public.calculate_enhanced_sensei_insights(p_sensei_id UUID)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
    ...
    FOR trip_record IN 
        SELECT id, theme, destination, dates FROM public.trips t
        WHERE t.is_active = TRUE 
        AND t.trip_status = 'approved'
        AND (t.sensei_id IS NULL OR t.backup_sensei_id IS NULL)
    LOOP
        SELECT * INTO match_result
        FROM public.calculate_enhanced_sensei_match_score(
            p_sensei_id, 
            trip_record.theme, 
