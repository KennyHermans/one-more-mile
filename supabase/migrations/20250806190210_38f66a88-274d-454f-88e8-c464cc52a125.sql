-- Updated admin_update_sensei_level function with defensive payload building and debugging
...
BEGIN
  ...
  IF NOT is_user_admin THEN
    RETURN jsonb_build_object(
      ...
      'debug_info', jsonb_build_object(
        ...
        'is_admin', COALESCE(is_user_admin, false)::boolean
      )
    );
  END IF;
  ...
  IF p_new_level IS NULL OR p_new_level NOT IN ('apprentice', 'journey_guide', 'master_sensei') THEN
    RETURN jsonb_build_object(
      ...
      'debug_info', jsonb_build_object('provided_level', COALESCE(p_new_level, 'null'))
    );
  END IF;
  ...
  IF NOT FOUND THEN
    RETURN jsonb_build_object(
      ...
      'debug_info', jsonb_build_object('sensei_id', p_sensei_id::text)
    );
  END IF;
  ...
  -- Build requirements payload defensively
  requirements_payload := jsonb_build_object(
    'timestamp', EXTRACT(EPOCH FROM now())::numeric,
    'updated_by', 'admin',
    'admin_override', p_admin_override::boolean,
    'trips_led', CASE 
                  WHEN sensei_record.trips_led IS NULL THEN 0
                  ELSE sensei_record.trips_led::int
                END,
    'rating', CASE 
                WHEN sensei_record.rating IS NULL 
                  OR sensei_record.rating::text ILIKE 'NaN' 
                  OR sensei_record.rating::text ILIKE 'Infinity'
                THEN 0.0
                ELSE sensei_record.rating::float
              END,
    'admin_user_id', admin_user_id::text
  );

  RAISE NOTICE 'Payload to insert into requirements_met: %', requirements_payload;
  ...
  -- Insert into level history with pre-built payload
