-- Fix the handle_sensei_level_change trigger function
-- The issue is that now() returns a timestamp which is not JSON-serializable
  ...
  IF TG_OP = 'UPDATE' AND OLD.sensei_level != NEW.sensei_level THEN
    INSERT INTO public.sensei_level_history (
      ...
      jsonb_build_object(
        ...
        'rating', NEW.rating,
        'timestamp', now()::text  -- ✅ Convert timestamp to text for JSON compatibility
      )
    );
  END IF;
