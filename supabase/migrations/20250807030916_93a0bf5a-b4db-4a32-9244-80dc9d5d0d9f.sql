...
SELECT tgname, tgrelid::regclass FROM pg_trigger WHERE tgrelid = 'trips'::regclass;

-- Drop all triggers on trips table
DO $$ 
DECLARE
    r RECORD;
BEGIN
    FOR r IN SELECT tgname FROM pg_trigger WHERE tgrelid = 'trips'::regclass
    LOOP
        EXECUTE 'DROP TRIGGER IF EXISTS ' || r.tgname || ' ON trips';
    END LOOP;
END $$;
