-- Fix EXCEPTION block to safely wrap values in COALESCE
    ...
    IF NOT p_admin_override THEN
        BEGIN
            ...
            IF eligibility_check ? 'error' THEN
                RETURN jsonb_build_object(
                    'success', false,
                    'error', 'Could not check sensei eligibility: ' || (eligibility_check->>'error')
                );
            END IF;
        ...
        EXCEPTION WHEN OTHERS THEN
            RETURN jsonb_build_object(
                ...
                'details', jsonb_build_object(
                    ...
                    'sqlerrm', COALESCE(SQLERRM, 'unknown')::text,
                    'sqlstate', COALESCE(SQLSTATE, '00000')::text
                )
            );
        END;
    END IF;
