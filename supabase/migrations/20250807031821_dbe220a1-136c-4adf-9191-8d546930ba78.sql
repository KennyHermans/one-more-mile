-- Enhanced Admin Trip Builder Schema Updates
...
CREATE TABLE IF NOT EXISTS trip_collaboration_comments (
  ...
  field_reference TEXT, -- Which field the comment refers to
...
CREATE TABLE IF NOT EXISTS sensei_availability_calendar (
  ...
  -- Constraint to prevent overlapping dates for same sensei
...
CREATE POLICY "Trip creators can view their trip workflow" ON trip_workflow_history
  FOR SELECT USING (
    trip_id IN (
      SELECT t.id FROM trips t 
      WHERE t.created_by_admin = auth.uid() OR 
            t.sensei_id IN (SELECT sp.id FROM sensei_profiles sp WHERE sp.user_id = auth.uid())
    )
  );
...
CREATE POLICY "Trip stakeholders can view comments" ON trip_collaboration_comments
  FOR SELECT USING (
    trip_id IN (
      SELECT t.id FROM trips t 
      WHERE t.created_by_admin = auth.uid() OR 
...
CREATE TRIGGER set_timestamp_trip_templates
  BEFORE UPDATE ON trip_templates
  FOR EACH ROW
  EXECUTE PROCEDURE trigger_set_timestamp();

CREATE TRIGGER set_timestamp_activity_library
  BEFORE UPDATE ON activity_library
  FOR EACH ROW
  EXECUTE PROCEDURE trigger_set_timestamp();

CREATE TRIGGER set_timestamp_collaboration_comments
  BEFORE UPDATE ON trip_collaboration_comments
  FOR EACH ROW
  EXECUTE PROCEDURE trigger_set_timestamp();

CREATE TRIGGER set_timestamp_sensei_availability
...
RETURNS TRIGGER AS $$
BEGIN
  -- Update auto-save timestamp when any data changes
